...
func main() {
	...
	flag.StringVar(&host, "host", "127.0.0.1", "worker host/address")
	...
	repo := workerpkg.NewPostgresRepository(pool)
	w := workerpkg.Worker{
		ID:      id,
		Status:  workerpkg.WorkerStatus(status),
		Address: host,
		Port:    port,
